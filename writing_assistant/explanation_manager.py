...
class ExplanationManager:
    ...
    def process_examples(self, word: str) -> None:
        ...
        try:
            # 在后台线程中获取数据库连接
            # 注意：SQLite连接和游标对象不能在不同线程之间共享
            conn = sqlite3.connect(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'translated.db')))
            ...
            self.current_examples = []
            self.current_song_stats = defaultdict(lambda: {'before': 0, 'after': 0})
            self.is_processing = False  # 确保在发生异常时，is_processing会被正确设置为False
        finally:
            # 强制垃圾回收以释放内存
            gc.collect()
